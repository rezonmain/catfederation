...
const APPLICATION_SECRET_LENGTH = 64;
...
const APPLICATION_DELETE_SCHEMA = z.object({
  applicationId: z.string().max(APPLICATION_ID_LENGTH),
});
const APPLICATION_UPDATE_SECRET_SCHEMA = z.object({
  applicationId: z.string().max(APPLICATION_ID_LENGTH),
});
...
export {
  ...
  APPLICATION_UPDATE_SECRET_SCHEMA,
};
